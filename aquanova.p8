...
sub = {
  ...
  deisired_heading = 200, -- target heading for manual helm control
  turn_rate = 5, -- degrees per second max turn rate
  speed = 0, -- knots 0-160
  deisired_speed = 0, -- target speed for helm control
  speed_acc = 5, -- knots per second acceleration/deceleration
  max_speed = 160,
  depth = 0, -- meters 0-12000
  desired_depth = 0, -- target depth for helm control
  depth_acc = 100, -- meters per second dive/surface rate
  max_depth = 12000,
  pitch = 0,
  roll = 0,
  docked = {
...
missions ={
  {id=1,msg="collect 3 bio samples in ao",sample_type="bio",reward=500,objs={},completed=false,transmitted=false},
  {id=2,msg="analyze mineral deposits",sample_type="min",reward=600,objs={},completed=false,transmitted=false}
}


-- sample sites (decimal degrees)
-- unified objectives (samples, waypoints, etc)
objs = {}
inv={} -- sample inventory
so=0 amd=0 tune=0 gain=0 atm=0

-- sample wave templates
sample_defs = {
  bio={waves={{offset=12,amp=3,tt=45,tg=3},{offset=67,amp=2,tt=45,tg=4},{offset=89,amp=4,tt=48,tg=3}},origin="volcanic",tags={"thermal","chemosyn"},rarity=1},
  min={waves={{offset=23,amp=2,tt=60,tg=2},{offset=45,amp=3,tt=58,tg=3}},origin="sediment",rarity=2},
  nrg={waves={{offset=5,amp=5,tt=30,tg=5},{offset=78,amp=3,tt=33,tg=4}},origin="geothermal",rarity=2},
  art={waves={{offset=90,amp=1,tt=80,tg=2}},origin="unknown",rarity=4}
}

function spawn_mission(m)
  local stype=m.sample_type or "bio"
  local def=sample_defs[stype]
  for i=1,3 do
    local o={typ=2,lon=-70+rnd(2),lat=40+rnd(2),rng=5,st=0,mid=m.id,stype=stype,waves=def.waves,name=stype.." sample #"..i,origin=def.origin,tags=def.tags,rarity=def.rarity}
    add(objs,o)
    add(m.objs,#objs)
  end
end
...
function update_missions()
  for m in all(missions) do
    if not m.completed then
      local cnt=0
      for s in all(inv) do
        if s.mid==m.id and s.st==2 then cnt+=1 end
      end
      if cnt>=3 then m.completed=true resources.money+=m.reward end
    end
  end
end
...
function _update()
  ...
  update_missions()
  if station=="science" and so>0 and so<=#inv and inv[so].st==1 and atm>0 then atm-=1 if atm==0 then local o=inv[so] if amd==1 or chk_lock(o) then o.st=2 resources.reputation+=o.rarity else o.st=3 resources.reputation-=o.rarity end amd=0 end end
end
...
function draw_link(x,y,lnk,mcel,opr) -- draw a data link box link and mcel number opr status
  if opr==true then
    opr="oprtve" c=10
  else
    opr="offlne" c=8
  ...
  line(x, y+6, x, y+32, 1)
  line(x+32, y, x+32, y+32, 1)
  line(x, y+6, x+12, y, 12)
  line(x+12, y, x+32, y, 12)
  line(x, y+32, x+32, y+32, 12)

  print("link"..lnk,x+12,y+2,12)
  print("mcel\fa"..mcel,x+1,y+8,12)
  print(opr, c)
  print("norpac", 10)
  print("46-9976", 10)

  rect(x, y+34, x+32, y+35, c)
end
  ...
  dpad(108, 108, {
    ...
    action = function()
      ...
      if active_waypoint_index == 0 then
        ...
        autopilot = true
        -- sync deisired_heading with waypoint bearing to prevent jump when autopilot disables
        sub.deisired_heading = calculate_bearing(sub.lon, sub.lat, world_lon, world_lat)
      end
    end
  })
end
...
function setup_helm_buttons()
  ...
  button("action", 86, 43, function()
    ...
    if autopilot then
      if #waypoints == 0 or active_waypoint_index == 0 then
        ...
        -- sync deisired_heading with current waypoint bearing
        local target_wpt = waypoints[active_waypoint_index]
        sub.deisired_heading = calculate_bearing(sub.lon, sub.lat, target_wpt.lon, target_wpt.lat)
      end
    else
      -- sync deisired_heading to current heading when disabling autopilot
      -- this prevents sudden turns when switching to manual control
      sub.deisired_heading = sub.heading
    end
  end)
  ...
  dpad(12, 43, {
    label = "heading",
    left = function()
      sub.deisired_heading -= 15
      if sub.deisired_heading < 0 then sub.deisired_heading += 360 end
      autopilot = false  -- manual heading change disables autopilot
    end,
    right = function()
      sub.deisired_heading += 15
      if sub.deisired_heading >= 360 then sub.deisired_heading -= 360 end
      autopilot = false  -- manual heading change disables autopilot
    end,
    up = function()
      sub.desired_depth -= 50
      if sub.desired_depth < 0 then sub.desired_depth = 0 end
    end,
    down = function()
      sub.desired_depth += 50
      if sub.desired_depth > sub.max_depth then sub.desired_depth = sub.max_depth end
    end
  })
  ...
  button("up", 34, 34, function()
    sub.deisired_speed += 5
    if sub.deisired_speed > sub.max_speed then sub.deisired_speed = sub.max_speed end
    -- autopilot remains active when changing speed
  end)
  button("down", 34, 43, function()
    sub.deisired_speed -= 5
    if sub.deisired_speed < 0 then sub.deisired_speed = 0 end
    -- autopilot remains active when changing speed
  end)
end
...
function setup_science_buttons()
  ...
  button("action",2,65,do_col)
  button("action",2,75,do_dis)
  button("up",2,85,do_prv)
  button("down",12,85,do_nxt)
  button("action",2,94,do_auto)
  dpad(72,74,{label="tune",left=function()do_tun(-2)end,right=function()do_tun(2)end,up=function()do_tun(10)end,down=function()do_tun(-10)end,action=do_ana})
  dpad(102,74,{label="gain",left=function()do_gain(-0.5)end,right=function()do_gain(0.5)end,up=function()do_gain(1)end,down=function()do_gain(-1)end,action=do_ana})
end
...
function update_ah() -- dynamic artaficial horizon
  ...
  -- pre-calculate rotation coefficients (used by all elements)
  local c_roll = cos(roll)
  local s_roll = sin(roll)
  ...
  for dy=-r,r do
    for dx=-r,r do
      if dx*dx+dy*dy<=r*r then
        -- apply rotation matrix: rotate by roll, then offset by pitch
        local world_y = dy + pitch_y
        local ry = dx * s_roll - world_y * c_roll
        if ry < 0 then pset(cx+dx, cy+dy, 3) end
      end
    end
  end

  -- draw horizon line (y=0 in world space)
  for dx = -r, r do
    local rx = dx * c_roll + pitch_y * s_roll
    local ry = dx * s_roll - pitch_y * c_roll
  ...
  for p = -20, 20, 5 do
    if p != 0 then
      ...
      local world_y = p + pitch_y
      for dx = -len, len do
        local rx = dx * c_roll + world_y * s_roll
        local ry = dx * s_roll - world_y * c_roll
...
function update_movement()
  ...
  local deisired_heading = sub.deisired_heading  -- default to manual helm heading
  ...
  if active_waypoint_index > 0 and active_waypoint_index <= #waypoints and autopilot then
    ...
    if dist < 5 and sub.speed > 0 then
      ...
      deisired_heading = calculate_bearing(sub.lon, sub.lat, target_wpt.lon, target_wpt.lat)
    end
  end

  -- === heading acceleration (turn rate limiting) ===
  -- gradually adjust heading toward target (autopilot or manual)
  local heading_diff = deisired_heading - sub.heading
  -- normalize to -180 to 180
  if heading_diff > 180 then heading_diff -= 360 end
  if heading_diff < -180 then heading_diff += 360 end

  -- adjust heading (turn_rate degrees per second max)
  -- at 30fps: turn_rate/30 degrees per frame
  local turn_rate_per_frame = sub.turn_rate / 30
  if abs(heading_diff) > turn_rate_per_frame then
    if heading_diff > 0 then
      sub.heading += turn_rate_per_frame
    else
      sub.heading -= turn_rate_per_frame
    end
  else
    sub.heading = deisired_heading
  end
  ...
  -- === speed acceleration ===
  -- gradually adjust speed toward desired speed
  local speed_diff = sub.deisired_speed - sub.speed
  local speed_acc_per_frame = sub.speed_acc / 30  -- knots per frame

  if abs(speed_diff) > speed_acc_per_frame then
    if speed_diff > 0 then
      sub.speed += speed_acc_per_frame
    else
      sub.speed -= speed_acc_per_frame
    end
  else
    sub.speed = sub.deisired_speed
  end

  -- clamp speed to valid range
  sub.speed = mid(0, sub.speed, sub.max_speed)

  -- === depth acceleration ===
  -- gradually adjust depth toward desired depth
  local depth_diff = sub.desired_depth - sub.depth
  local depth_acc_per_frame = sub.depth_acc / 30  -- meters per frame

  if abs(depth_diff) > depth_acc_per_frame then
    if depth_diff > 0 then
      sub.depth += depth_acc_per_frame
    else
      sub.depth -= depth_acc_per_frame
    end
  else
    sub.depth = sub.desired_depth
  end

  -- clamp depth to valid range
  sub.depth = mid(0, sub.depth, sub.max_depth)

  -- update pitch based on desired depth change (how steep the dive/climb is)
  -- pitch should reflect the dive planes setting, not instantaneous depth change
  -- normalize depth_diff to a reasonable pitch angle range
  -- diving (positive depth_diff) = nose down (positive pitch)
  -- surfacing (negative depth_diff) = nose up (negative pitch)
  local depth_diff = sub.desired_depth - sub.depth
  sub.pitch = mid(-20, depth_diff * 0.05, 20)  -- scale to +/- 20 degrees max
...
-- science functions
function rst() atm=0 tune=0 gain=0 end
function do_col() for i,o in pairs(objs) do if o.typ==2 and o.st==0 and calculate_distance(sub.lon,sub.lat,o.lon,o.lat)<o.rng then del(objs,o) add(inv,o) so=#inv rst() return end end end
function do_prv() if #inv==0 or so<=1 then return end so-=1 rst() end
function do_nxt() if #inv==0 or so>=#inv then return end so+=1 rst() end
function do_tun(d) if amd==0 then tune=mid(0,tune+d,100) end end
function do_gain(d) if amd==0 then gain=mid(1,gain+d,10) end end
function do_dis() if so>0 and so<=#inv then del(inv,inv[so]) if so>#inv then so=#inv end rst() end end
function do_ana() if so>0 and so<=#inv and inv[so].st==0 and resources.supplies>=1 then resources.supplies-=1 inv[so].st=1 inv[so].lt=tune inv[so].lg=gain atm=300 end end
function do_auto() if so>0 and so<=#inv and inv[so].st==0 and resources.supplies>=2 then resources.supplies-=2 inv[so].st=1 atm=inv[so].rarity*1800 amd=1 end end
function chk_lock(o) for w in all(o.waves) do if abs(o.lt-w.tt)>5 or abs(o.lg-w.tg)>1 then return false end end return true end
...
function draw_communications()
  ...
  draw_ui_buttons()
  -- draw comm links
  draw_link(0,0,1,2309, true)
  draw_link(34,0,2,713, false)
  draw_link(68,0,3,4537, true)

end

function draw_sensors()
  -- placeholder for sensors station
  print("sensors", 101, 2, 7)
...
function draw_bridge()
  ...
  -- pre-calculate grid offset to avoid repeated modulo operations
  local lon_offset = (sub.lon % 1.0) * zoom
  local lat_offset = (sub.lat % 1.0) * zoom
  local grid_step = zoom  -- 1 degree in pixels

  -- draw vertical lines (longitude)
  for i = -3, 3 do
    local x = 47 + (i * grid_step - lon_offset)
    line(x, 0, x, map_size, 1)
  end

  -- draw horizontal lines (latitude)
  for i = -3, 3 do
    local y = 64 - (i * grid_step - lat_offset)
  ...
  for o in all(objs) do
    local x,y=world_to_screen(o.lon,o.lat)
    local spr_id=o.typ==1 and 34 or (o.typ==2 and 35 or 36)
    local col=o.st==0 and 11 or (o.st==1 and 10 or 8)
    spr(spr_id,x-4,y-4)
    if o.st==2 then circfill(x,y,2,col) end
  end

  -- draw waypoint route lines and markers
  if #waypoints > 0 then
    -- cache first waypoint position
    local prev_x, prev_y = world_to_screen(waypoints[1].lon, waypoints[1].lat)

    -- draw line from ownship to first waypoint
    draw_dashed_line(47, 64, prev_x, prev_y, 10)

    -- draw first waypoint marker
    local color = (1 == active_waypoint_index) and 11 or 10
    circ(prev_x, prev_y, 2, color)
    print(1, prev_x-1, prev_y-2, color)

    -- draw remaining waypoints and lines in single loop
    for i=2,#waypoints do
      local curr_x, curr_y = world_to_screen(waypoints[i].lon, waypoints[i].lat)

      -- draw line from previous waypoint to current
      draw_dashed_line(prev_x, prev_y, curr_x, curr_y, 9)

      -- draw waypoint marker
      color = (i == active_waypoint_index) and 11 or 10
      circ(curr_x, curr_y, 2, color)
      print(i, curr_x-1, curr_y-2, color)
  ...
  print("bridge", 101, 2, 7) -- station title
  draw_waypoint_info()
...
function draw_helm()
  ...
  draw_scale(22,104,20,mid(-1,p+r*0.3,1),"sd")
  print("aft steering",65,98,12)
  draw_scale(44,104,20,mid(-1,p+r*0.2,1),"1")
  draw_scale(64,104,20,mid(-1,p-r*0.2,1),"2")
  draw_scale(84,104,20,mid(-1,p+r*0.2,1),"3")
  draw_scale(104,104,20,mid(-1,p-r*0.2,1),"4")
...
function draw_science()
  ...
  local y=14
  local o=so>0 and so<=#inv and inv[so] or nil
  if o then
    print("sample: "..o.name,2,8,11)
    local st_txt=o.st==0 and "ready" or (o.st==1 and "analyzing" or (o.st==2 and "success" or (o.st==3 and "failed" or "none")))
    local st_col=o.st==0 and 7 or (o.st==1 and 10 or (o.st==2 and 11 or 8))
    print(st_txt,4,57,st_col)
    rect(2,y,96,y+49,7)
    if o.waves then
      local wc=o.st==2 and 11 or (o.st==3 and 8 or 7)
      for w in all(o.waves) do
        local te=abs(tune-w.tt)/5
        local ge=abs(gain-w.tg)/1
        local sig=max(0,1-te)*max(0,1-ge)
        for x=0,93 do
          local h=sin((x+tune+w.offset)/16)*sig*8
          pset(3+x,y+25+h,wc)
        end
      end
    end
    if o.st==1 then print("timer:"..flr(atm/30),50,57,10) end
  end
  local in_rng=false
  for i,obj in pairs(objs) do
    if obj.typ==2 and obj.st==0 and calculate_distance(sub.lon,sub.lat,obj.lon,obj.lat)<obj.rng then
      in_rng=true break
    end
  end
  print(in_rng and "in range" or "out range",66,26,in_rng and 11 or 8)
  print("slot:"..so.."/"..#inv,66,18,7)
  print("collect", 12,67,7)
  print("discard", 12,77,7)
  print("auto",12,96,7)
  print("tune:"..flr(tune),65,91,7)
  print("gain:"..flr(gain*10)/10,94,91,7)
  print("supplies:"..resources.supplies,66,113,7)
  draw_ui_buttons()
end

function draw_engineering()
  print("engnrng",101,2,7)
...

__map__

